function Player (cash, happy, grade, energy, loan) {
    this.cash = cash;
    this.happy = happy;
    this.grade = grade;
    this.energy = energy;
	...
	this.updateFirst = function(type, answerIndex) {
		switch (type)
		{
			case 0:
				if (answerIndex == 0)
	...
	this.updateRan = function(type, answerIndex) {
		message = "";
		errorMessage = "";
		var x = takeExam();
		if (takeExam)
	...
	this.updateDepend = function(type, answerIndex) {
		message = "";
		errorMessage = "";
		var y = takeExam();
		if (takeExam)
		...
		} else
		{
			...
			case 0:
				if (answerIndex == 0)
				{
					if (grades >= 95)
					{
						int x = 500 + Math.floor((Math.random()*4500)+1);;
						cash += x;
						happy += 20;
					...
					{
						int x = 500 + Math.floor((Math.random()*1500)+1);;
						cash += x;
						happy += 15;
						message = "You won a $" + x + " scholarship, congrats!";
					}
	...
	this.addCash = function(x){
		cash += x;
	}
	
	this.addHappy = function(x){
		happy += x;
	}
	
	this.addGrades = function(x){
		grades += x;
	}
	
	this.addEnergy = function(x){
		energy += x;
	}
	
	this.addWeek = function(x){
		week += x;
	}
	...
	this.constantExpenses = function(num, updateNum){
